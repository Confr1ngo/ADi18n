...
export function addRealityTime(time, realTime, rm, level, realities, ampFactor, projIM) {
  let reality = "";
  const celestials = [Teresa, Effarig, Nameless, V, Ra, Laitela];
...
export function getGameSpeedupFactor(effectsToConsider, blackHolesActiveOverride) {
  ...
  if (Nameless.isStoringGameTime && effects.includes(GAME_SPEED_EFFECT.TIME_STORAGE)) {
...
export function getGameSpeedupForDisplay() {
  ...
  if (
    Nameless.isAutoReleasing &&
    Nameless.canRelease(true) &&
    !BlackHoles.areNegative &&
    !Pelle.isDisabled("blackhole")
  ) {
    return Math.max(Nameless.autoReleaseSpeed, speedFactor);
  }
  return speedFactor;
}
...
export function realTimeMechanics(realDiff) {
  ...
  Ra.memoryTick(realDiff, !Nameless.isStoringRealTime);
...
export function gameLoop(passDiff, options = {}) {
  ...
  if (Nameless.isAutoReleasing) {
    Nameless.autoReleaseTick++;
  }
  if (Nameless.autoReleaseTick >= 5) {
    Nameless.autoReleaseTick = 0;
    Nameless.useStoredTime(true);
    Nameless.isReleaseTick = true;
  } else if (!Nameless.isReleaseTick) {
    Nameless.nextTickDiff = realDiff;
  }
  if (diff === undefined) {
    diff = Nameless.nextTickDiff;
  }
  ...
  if (!Nameless.isReleaseTick && !fixedSpeedActive) {
    let speedFactor;
    ...
    if (Nameless.isStoringGameTime && !fixedSpeedActive) {
      ...
      const beforeStore = player.celestials.nameless.stored;
      player.celestials.nameless.stored = Math.clampMax(player.celestials.nameless.stored +
        diff * (totalTimeFactor - reducedTimeFactor) * amplification, Nameless.timeCap);
      Nameless.currentBlackHoleStoreAmountPerMs = (player.celestials.nameless.stored - beforeStore) / diff;
      speedFactor = reducedTimeFactor;
    }
    diff *= speedFactor;
  } else if (fixedSpeedActive) {
    diff *= getGameSpeedupFactor();
    Nameless.currentBlackHoleStoreAmountPerMs = 0;
  }
  player.celestials.ra.peakGamespeed = Math.max(player.celestials.ra.peakGamespeed, getGameSpeedupFactor());
  Nameless.isReleaseTick = false;
  ...
  if (!Achievement(188).isUnlocked) {
    ...
    player.records.thisEternity.realTime += realDiff;
    if (Nameless.isRunning && Nameless.feltEternity && !EternityChallenge(12).isRunning) {
  ...
  if (Nameless.canTickHintTimer) {
    player.celestials.nameless.hintUnlockProgress += Nameless.isRunning ? realDiff : (realDiff * 0.4);
    if (player.celestials.nameless.hintUnlockProgress >= TimeSpan.fromHours(5).totalMilliseconds) {
      NamelessProgress.hintsUnlocked.giveProgress();
      Nameless.quotes.hintUnlock.show();
    }
  }
  ...
  if (!Nameless.canAmplify) {
    Nameless.boostReality = false;
  }
...
export function getTTPerSecond() {
  ...
  // Glyph TT generation
  const glyphTT = Teresa.isRunning || Nameless.isRunning || Pelle.isDoomed
    ? 0
