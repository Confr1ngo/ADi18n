export default {
  ...
  computed: {
    formatMachinesGained() {
      if (this.machinesGained.gt(0)) return $t("gain_X_rm", format(this.machinesGained, 2));
      return $t("no_machines_gained");
    },
    formatMachineStats() {
      ...
      if (this.machinesGained.gt(0) && this.machinesGained.lt(100)) {
        return `(${$t("next_rm_at_X_ep", format(this.nextMachineEP, 2))})`;
      }
      if (this.machinesGained.eq(0) && this.newIMCap === 0) {
        return `(${$t("projected_X_rm", format(this.projectedRM, 2))})`;
      }
      if (this.newIMCap !== 0) {
        return `(${$t("im_cap_X", formatMachines(0, this.newIMCap))})`;
      }
  ...
  methods: {
    ...
    hasSpecialReward() {
      ...
      return Currency.eternityPoints.value.exponent > 4000 &&
        ((Effarig.isRunning && !EffarigUnlock.reality.isUnlocked) || (Nameless.isRunning && !Nameless.isCompleted));
    }
  },
  template: `
  <div class="l-reality-button">
    ...
    >
      <div class="l-reality-button__contents">
        ...
        <template v-else>
          <div>{{ $t("purchase_reality_study_to_unlock_reality") }}</div>
        </template>
        <div
          v-if="canReality"
