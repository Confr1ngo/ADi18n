...
export default {
  ...
  computed: {
    ...
    physicsText() {
      ...
      if (this.physicsOverride === undefined) {
        ...
        return $t("X_fixed", enableStr);
      }
    },
    perkText() {
      return $p_split("you_have_X_perk_points", this.pp, formatInt(this.pp));
    }
  },
  created() {
  ...
  >
    {{ perkText[0] }} <span class="c-perk-tab__perk-points" data-v-perk-point-label>{{ perkText[1] }}</span> {{ perkText[2] }}
    <br>
