...
import NamelessTimeStudy from "./NamelessTimeStudy.js";
...
export default {
  ...
  created() {
    ...
    // CSS controlling the fade in/out for the Nameless study is an animation happening over the course of 1 second.
    ...
    this.on$(GAME_EVENT.REALITY_RESET_AFTER, () => {
      this.delayTimer = player.celestials.nameless.hasSecretStudy
        ? Date.now()
        : 0;
    });
  ...
  methods: {
    update() {
      ...
      this.isNameless = Nameless.isRunning || Date.now() - this.delayTimer < 1000;
    },
  ...
  <div class="l-time-studies-tab">
    ...
    >
      ...
      >
        ...
        <HiddenTimeStudyConnection
          v-if="isNameless"
          :setup="layout.namelessStudyConnection"
          :is-nameless="isNameless"
        />
      </svg>
    </div>
  </div>
