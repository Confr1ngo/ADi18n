...
class BlackHoleState {
  ...
  get isUnlocked() {
    return this._data.unlocked && !Nameless.isRunning && !Pelle.isDisabled("blackhole");
  }
  ...
  get displayState() {
    if (Pelle.isDisabled("blackhole")) return `<i class="fas fa-ban"></i> ${$t("disabled")}`;
    if (Nameless.isAutoReleasing) {
      if (Nameless.autoReleaseTick < 3) return `<i class="fas fa-compress-arrows-alt u-fa-padding"></i> ${$t("pulsing")}`;
      return `<i class="fas fa-expand-arrows-alt u-fa-padding"></i> ${$t("pulsing")}`;
    }
    if (Nameless.isStoringGameTime) return `<i class="fas fa-compress-arrows-alt"></i> ${$t("charging")}`;
    if (BlackHoles.areNegative) return `<i class="fas fa-caret-left"></i> ${$t("inverted")}`;
    if (BlackHoles.arePaused) return `<i class="fas fa-pause"></i> ${$t("paused")}`;
    if (this.isPermanent) return `<i class="fas fa-infinity"></i> ${$t("permanent")}`;

    const timeString = TimeSpan.fromSeconds(this.timeToNextStateChange).toStringShort(true);
    if (this.isActive) return `<i class="fas fa-play"></i> ${$t("active_X", timeString)}`;
    return `<i class="fas fa-redo"></i> ${$t("inactive_X", timeString)}`;
  }
    ...
    if (this.isCharged) {
      if (this.phase >= this.duration) {
        ...
        if (GameUI.notify.showBlackHoles) {
          GameUI.notify.blackHole($t("X_duration_ended", this.description(true)));
        }
      }
      ...
      if (GameUI.notify.showBlackHoles) {
        GameUI.notify.blackHole($t("X_has_activated", this.description(true)));
      }
    }
  }
  ...
  description(capitalized) {
    if (RealityUpgrade(20).isBought) {
      return $t("black_hole_X", this.id.toString());
    }
...
export const BlackHoles = {
  ...
  togglePause: (automatic = false) => {
    ...
    if (ImaginaryUpgrade(24).isLockingMechanics && Ra.isRunning && maxInversion) {
      if (!automatic) ImaginaryUpgrade(24).tryShowWarningModal($t("imaginary_upgrade_23_lock_event3"));
      return;
    }
    ...
    const blackHoleString = RealityUpgrade(20).isBought ? "Black Holes" : $t("black_hole");
