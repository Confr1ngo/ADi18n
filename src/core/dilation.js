  ...
  if (id > 3 && id < 11) {
    ...
    if (id === 3 && !Pelle.isDisabled("tpMults")) {
      ...
      if (Nameless.isRunning) {
        retroactiveTPFactor = Math.pow(retroactiveTPFactor, Nameless.tachyonNerf);
      }
...
export function getDilationGainPerSecond() {
  ...
  if (Nameless.isRunning && !dtRate.eq(0)) dtRate = Decimal.pow10(Math.pow(dtRate.plus(1).log10(), 0.85) - 1);
...
export function tachyonGainMultiplier() {
  ...
  const pow = Nameless.isRunning ? Nameless.tachyonNerf : 1;
...
export function getBaseTP(antimatter, requireEternity) {
  ...
  if (Nameless.isRunning) baseTP = baseTP.pow(Nameless.tachyonNerf);
  return baseTP;
}
...
export function getTachyonReq() {
  ...
  if (Nameless.isRunning) effectiveTP = effectiveTP.pow(1 / Nameless.tachyonNerf);
...
export function getDilationTimeEstimate(goal) {
  ...
  if (PelleRifts.paradox.isActive) {
    ...
    if (goalNetRate.lt(0)) return "由于填充裂隙而永远无法购买";
